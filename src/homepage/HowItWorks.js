...
const HowItWorks = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 lg:px-8">
      ...
      {/* Full Screen Modal with Bottom Caption */}
      {isFullScreen && (
        <div className="fixed inset-0 z-50 bg-white flex flex-col items-center justify-center">
          <button
            onClick={handleCloseFullScreen}
            className="absolute top-4 right-4 md:top-8 md:right-8 text-white hover:text-gray-300 transition-colors z-50 bg-black bg-opacity-50 p-2 rounded-full"
            aria-label="Close full screen"
          >
            <MdClose className="w-8 h-8 md:w-10 md:h-10" />
          </button>
          
          {/* Full Screen Image - Takes most of the screen */}
          <div className="w-full h-[70vh] md:h-[70vh] flex items-center justify-center">
            <img
              src="imgs/Retrofit-house.jpg" 
              alt="United Nations Sustainable Development Goals icons grid" 
              className="w-full h-full object-contain"
            />
          </div>
          
          {/* Caption Panel at Bottom - Takes remaining space */}
          <div className="w-full h-[30vh] md:h-[30vh] bg-gray-900 text-white overflow-y-auto">
            <div className="max-w-7xl mx-auto p-6 md:p-8">
              <h2 className="text-xl md:text-2xl lg:text-3xl font-bold mb-4">
                Save Money and Gain Confidence with RENEW Eligibility Check
              </h2>
              <div className="space-y-3">
                ...
                <p className="text-sm md:text-base font-semibold italic" style={{color: '#4ade80'}}>
      ...
      <div className="max-w-7xl mx-auto mt-20 px-6 lg:px-8">
        ...
        {/* Main Content - Full width image with caption overlay */}
        <div className="relative w-full">
          {/* Full width image container */}
          <div className="w-full relative cursor-pointer" onClick={handleOpenFullScreen}>
            <img
              src="imgs/Retrofit-house.jpg" 
              alt="United Nations Sustainable Development Goals icons grid" 
              className="w-full h-auto max-h-[500px] md:max-h-[600px] object-cover rounded-lg shadow-2xl"
            />
            
            {/* Fullscreen button overlay */}
            <div className="absolute top-4 right-4 bg-grey-900 bg-opacity-50 text-white p-2 rounded-full hover:bg-opacity-70 transition-all">
              <MdFullscreen className="w-5 h-5 md:w-6 md:h-6" />
            </div>
          </div>
          
          {/* Caption overlay - positioned on top of image */}
          <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/70 via-black/40 to-transparent p-4 md:p-6 text-white cursor-pointer" onClick={handleOpenFullScreen}  >
            <div className="max-w-3xl mx-auto">
              <div className="text-lg md:text-xl lg:text-xl font-bold mb-2">
                A common pain point for both users and suppliers of renewable energy products ...
              </div>
              
              {/* Click hint - smaller and more subtle */}
              <div className="flex items-center gap-1 text-xs md:text-sm text-green-300 font-medium">
                <span className="truncate">Click image to view full details</span>
                <MdFullscreen className="w-3 h-3 md:w-4 md:h-4 flex-shrink-0" />
              </div>
            </div>
          </div>
        </div>
